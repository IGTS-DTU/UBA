...
const PersonalScoreUBA = () => {
  ...
  useEffect(() => {
    ...
    const storedEmail = "gaurav.gp176@gmail.com"; //used it for testing
  ...
  const findUserPool = async (email) => {
    try {
      for (let pool of POOLS) {
        ...
        const usersRef = doc(db, "IGTS", "uba", poolDoc, "users");
  ...
  const fetchUserScores = async (pool, index) => {
    try {
      const poolDoc = `pool${POOLS.indexOf(pool) + 1}`;
      const scoresRef = doc(db, "IGTS", "uba", poolDoc, "scores");
      ...
      if (scoresSnap.exists()) {
        ...
        const userScores = ["round1", "round2", "round3"].map(
          (round) => (allScores[round]?.[index] ?? "-") === 0 ? 0 :
            parseFloat(allScores[round]?.[index]) ?? "-"
          );
  ...
  const fetchFinalScore = async (pool, index) => {
    try {
      const poolDoc = `pool${POOLS.indexOf(pool) + 1}`;
      const finalScoresRef = doc(db, "IGTS", "uba", poolDoc, "finalScores");
